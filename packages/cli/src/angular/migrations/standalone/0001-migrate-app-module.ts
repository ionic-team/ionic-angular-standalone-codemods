...
export const migrateAppModule = async (project: Project, cliOptions: CliOptions) => {
  ...
  const ngModuleDecorator = appModule.getClass('AppModule')?.getDecorator('NgModule');
  ...
  const importsProperty = ngModuleDecoratorArguments.getDescendantsOfKind(
  ...
  if (importsProperty) {
    const importsPropertyAssignment = importsProperty.getInitializerIfKind(SyntaxKind.ArrayLiteralExpression);
    ...
    if (importsPropertyAssignment) {
      ...
      if (ionicModule) {
        ...
        ionicModuleForRootCallExpression = ionicModule.compilerNode as ts.CallExpression;
  ...
  let providersProperty = ngModuleDecoratorArguments.getDescendantsOfKind(
    SyntaxKind.PropertyAssignment
  ...
  if (!providersProperty) {
    ...
    providersProperty = ngModuleDecoratorArguments.asKind(SyntaxKind.ObjectLiteralExpression)!.addPropertyAssignment({
      name: 'providers',
      initializer: '[]'
    });
  }

  const providersPropertyAssignment = providersProperty.getInitializerIfKind(SyntaxKind.ArrayLiteralExpression);
